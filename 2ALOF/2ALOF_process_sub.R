...
{	    
	    ...
	    if(length(hp_het)>1) ## need at least two variants that are heterouzygous
	    {
...
#    hap1_num_dat[h1==G_MAT2$MAJ_A[j],j] = 0

    hap2_num_dat[h2==G_MAT2$MIN_A[j],j] = 1
#    hap2_num_dat[h2==G_MAT2$MAJ_A[j],j] = 0
    g_num_dat[,j] = hap1_num_dat[,j] + hap2_num_dat[,j]
  }
 
  ##g_num_dat = hap1_num_dat + hap1_num_dat
...
#data_merge <- function(datin1,datin2)
#{
#}
