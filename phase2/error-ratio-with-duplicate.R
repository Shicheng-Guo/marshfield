...
input=data.frame(matrix(as.vector(Rlt[,1]),byrow=T,ncol=2),rlt)
write.table(input,file="duplicates.inconsistency.rate.txt",sep="\t",quote=F,col.names = c("Sam1","Sam2","Inconsistency"),row.names=F)
